---
title: 'Prolific Calibration V313 6pics - 2500ms analysis'
output:
github_document:
toc: yes
toc_float: yes
...
```{r}  
removeSubdp = basic$subID[basic$d_p<d_p_thre]
...
####stats
```{r}
anovaFeature3rdRun = aov(d_p~feature_index,filter(basicFeature,run==3))
summary(anovaFeature3rdRun)


summarize = dplyr::summarize
compareFeature3rdRun = basicFeature  %>%filter(run==3) %>% group_by(feature_index) %>%summarize(meanD_p = mean(d_p),std =sd(d_p),med = median(d_p) )
