---
title: 'Prolific Calibration V32627 6pics - 2500ms analysis'
output:
github_document:
toc: yes
toc_float: yes
...
```{r}
time = '2500';
pilot_version = "70levels_V32627F";
...
#top 10 subjects
topdp = quantile(basicFeature$d_p,70/100)
...
summarize(  accuracy = sum(accuracy)/length(subID),
  ...
  ggplot(filter(basicLevelFeature,feature_index==26), aes(x=level_diff,y=accuracy))+
    geom_boxplot(fill = "white",lwd = 1)+
    geom_jitter(width=0.2,alpha = 0.5)+
    labs(title="Feauture 26", x = "level diff", y = "accuracy")+
    ylim(0,1)+
    plotformat
  
  ggplot(filter(basicLevelFeature,feature_index==27), aes(x=level_diff,y=accuracy))+
    geom_boxplot(fill = "white",lwd = 1)+
    geom_jitter(width=0.2,alpha = 0.5)+
    labs(title="Feauture 27", x = "level diff", y = "accuracy")+
    ylim(0,1)+
    plotformat
...
summarize(  accuracy = sum(accuracy)/length(subID),
  ...
  ggplot(filter(basicLevelFeature,feature_index==26), aes(x=level_diff,y=accuracy,group = level_diff,color=subID))+
    geom_boxplot(fill = "white",lwd = 1)+
    geom_jitter(width=0.2,alpha = 0.5)+
    labs(title="Feauture 26", x = "level diff", y = "accuracy")+
    ylim(0,1)+
  ...
  ggplot(filter(basicLevelFeature,feature_index==27), aes(x=level_diff,y=accuracy,group = level_diff,color=subID))+
    geom_boxplot(fill = "white",lwd = 1)+
    geom_jitter(width=0.2,alpha = 0.5)+
    labs(title="Feauture 27", x = "level diff", y = "accuracy")+
    ylim(0,1)+
...
accuracy_thre = quantile(basic$d_p[!is.na(basic$d_p)],prob = d_p_thre)
removeSubdp = basic$subID[basic$d_p<removedp_thre | is.na(basic$d_p)]
sprintf("low d-prime %f. subjects: %i",removedp_thre,length(removeSubdp))
