---
title: 'Prolific Calibration V328 6pics - 2500ms analysis'
output:
github_document:
toc: yes
toc_float: yes
...
```{r}
time = '2500';
pilot_version = "70levels_V328";
dataPath = '/Volumes/Macintosh HD/Users/Pam_sf_wang/Documents/Perceptual_learning_project/Calibration/Calibration_analysis/Prolific_analysis/90levels/';
...
basic = totalData %>% filter(keys>=0, run==3)%>% group_by(subID)%>% summarize(
  cr = sum(keys==1&SameDiff=="S"), 
  miss= sum(keys==1&SameDiff=="D"), 
  hit = sum(keys==0&SameDiff=="D"),
  fa = sum(keys==0&SameDiff=="S"),
  HitRate =hit/sum(SameDiff=="D"), 
  FARate = fa/sum(SameDiff=="S"), 
  CRRate = cr/sum(SameDiff=="S"),
  MissRate = miss/sum(SameDiff=="D"), 
  adjFARate = 1/(2*sum(SameDiff=="S")),
  adjHitRate = 1-(1/(2*sum(SameDiff=="D"))))
...
summarize(  accuracy = sum(accuracy)/length(subID),
            ...
            FARate = fa/sum(SameDiff=="S"), 
            CRRate = cr/sum(SameDiff=="S"),
            MissRate = miss/sum(SameDiff=="D"))  #filter(keys>=0)%>%
  basicLevelFeature_longform = gather(basicLevelFeature, response_type, resp_rate, HitRate,FARate,CRRate,MissRate)
  head(basicLevelFeature_longform)
