...
longform_HitRT = gather(hitRT, response_type, RT, hitF1,hitF2,hitF3)
ggplot(longform_HitRT, aes(x = response_type, y = RT, color = response_type))+
  geom_boxplot(fill = "white",lwd = 1)+
  geom_jitter(width=0.2,alpha = 0.5)+
  labs(title="Hit RT: features", x = "response type", y = "RT(ms)")+
  scale_color_manual(values=Features[c(4,8,12)])+
  plotformat

meanhitRT = hitRT %>%summarize(meanhitF1 = mean(hitF1), meanhitF2 = mean(hitF2), meanhitF3 = mean(hitF3))
meanhitRT
...
meanfa = basicFeature %>%summarize( meanfaF0F1Rate = mean(faF0F0Rate),
                                    meanfaF0F2Rate = mean(faF0F2Rate),
                                    meanfaF0F3Rate = mean(faF0F3Rate),
                                    meanfaF0F0Rate = mean(faF0F0Rate),
                                    meanfaF1F0Rate = mean(faF1F0Rate),
                                    meanfaF1F2 = mean(faF1F2Rate), 
                                    meanfaF1F3 = mean(faF1F3Rate),
                                    meanfaF1F1 = mean(faF1F1Rate),
                                    meanfaF2F0 = mean(faF2F0Rate),
                                    meanfaF2F1 = mean(faF2F1Rate),
                                    meanfaF2F3 = mean(faF2F3Rate),
                                    meanfaF2F2 = mean(faF2F2Rate),
                                    meanfaF3F0 = mean(faF3F0Rate),
                                    meanfaF3F1 = mean(faF3F1Rate),
                                    meanfaF3F2 = mean(faF3F2Rate),
                                    meanfaF3F3 = mean(faF3F3Rate))
meanfa
