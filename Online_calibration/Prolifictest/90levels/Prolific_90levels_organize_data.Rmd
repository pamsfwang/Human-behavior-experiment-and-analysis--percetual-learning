---
title: '70levels_V3 data organization'
output:
github_document:
toc: yes
toc_float: yes
...
data_new_fname = paste0("Prolific_70levels_",calibration_version,"_",time)
demographic_new_fname = paste0("Prolific_70levels_",calibration_version,"_demographic",time)
postsurvey_new_fname = paste0("Prolific_70levels_",calibration_version,"_postsurvey",time)
```
...
runFnamepatter = "SameDifferentRun01"
...
for (ifiles in 1:length(SameDifferentRun01_fnames)){
  ##Getting post-test data    
    ...
    if (unlist(strsplit(SameDifferentRun01_fnames[ifiles],runFnamepatter))[2]==unlist(strsplit(Post_fnames[ifilesDe],"postsurvey"))[2]){   
      tempPostFile = read.table(paste0(dataPath,Post_fnames[ifilesDe]),header = F)
        ...
        confirmationCode[ic,1]=unlist(strsplit(SameDifferentRun01_fnames[ifiles],runFnamepatter ))[2]
        }#if 
        }#for
  
   #Getting demo data
  for (ifilesDe in 1:length(Demo_fnames)){
      if (unlist(strsplit(SameDifferentRun01_fnames[ifiles],runFnamepatter))[2]==unlist(strsplit(Demo_fnames[ifilesDe],"demographics"))[2]){  tempDemoFile = read.table(paste0(dataPath,Demo_fnames[ifilesDe]),header = F)
...
```{r}
runNum = 2;
runFnamepatter = "SameDifferentRun02"
dataRun02 = data.frame()
##Getting data from online text output
for (ifiles in 1:length(SameDifferentRun02_fnames)){
  
  for (ifilesDe in 1:length(Post_fnames)){
    if (unlist(strsplit(SameDifferentRun02_fnames[ifiles],runFnamepatter))[2]==unlist(strsplit(Post_fnames[ifilesDe],"postsurvey"))[2]){tempPostFile = read.table(paste0(dataPath,Post_fnames[ifilesDe]),header = F)
        ###separate feedback and prolific ID
        if (dim(tempPostFile)[2]>1){
          tempPostFile$y = apply( tempPostFile[,] , 1 , paste , collapse = "" )
          Post_info = unlist(strsplit(as.character(tempPostFile$y), ";"))
...
##########
  ...
  feature_index = data.frame()
    ...
    dataRun02 = rbind(dataRun02,data)
    totalData = rbind(totalData,data)
    #}else{}
}#for
