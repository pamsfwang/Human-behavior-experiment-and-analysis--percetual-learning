---
title: 'Prolific V323 categorization analysis Day01-implicity object/category match'
output:
github_document:
toc: yes
toc_float: yes
...
- 4 real runs in total.   
- The first two runs are one-back task on Object match: first run they only saw objects from one category. The second run they saw objects only from the other category. Their task is to indicate whether the current object is the same as the obect presented one trial ago. They saw each object 10 times. With feedback            
- he last two runs are one-back task on Category match: the third and fourth runs, they saw objects from both categories. Their task is to indicate whether the current object is in the same category as the obect presented one trial ago. They saw each object 8 times. With feedback       
...
##Task structure
```{r}
temptrialNumbers=totalData %>%filter(run==1 |run==2) %>%group_by(subID,SameDiffImg)%>%summarize(num_same_Img = length(SameDiffImg))
temptrialNumbers

temptrialNumbersCat=totalData %>%filter(run==3 |run==4) %>% group_by(subID,SameDiffCat)%>%summarize(num_same_Cat = length(SameDiffCat))
temptrialNumbersCat
```


```{r}
category_perform = totalData %>% filter(accuracy!=-1)%>%group_by(subID,run) %>% summarize(accuracy_rate = sum(accuracy)/length(subID))
...
basic = totalData %>% filter(run==4)%>% group_by(subID)%>% summarize(
...
summarize = dplyr::summarize
##########
basicFeature = totalData%>% filter(run==2 | run ==1)%>%group_by(subID,run)%>% summarize(
