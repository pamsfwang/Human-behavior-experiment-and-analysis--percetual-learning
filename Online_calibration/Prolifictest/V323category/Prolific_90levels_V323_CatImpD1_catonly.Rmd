---
title: 'Prolific V323 categorization analysis Day01-implicity category match'
output:
github_document:
toc: yes
toc_float: yes
...
- four runs of one-back task on Category match: the third and fourth runs, they saw objects from both categories. Their task is to indicate whether the current object is in the same category as the obect presented one trial ago. They saw each object 8 times. With feedback         
...
##Task structure
```{r}
temptrialNumbersCat=totalData%>% group_by(subID,SameDiffCat,run)%>%summarize(num_same_Cat = length(SameDiffCat))
temptrialNumbersCat
```
...
#d-p
ggplot(basic, aes(x = run, y = d_p, color = as.factor(subID)))+
  geom_point(size=3)+
