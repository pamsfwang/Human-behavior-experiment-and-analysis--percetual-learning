---
title: 'Prolific Calibration V323 categorization explicit (SDobj_cat)'
output:
github_document:
toc: yes
toc_float: yes
...
- The first two runs are one-back task on Object match: first run they only saw objects from Category 1. The second run they saw objects only from Category 2 (explicitly told). Their task is to indicate whether the current object is the same as the obect presented one trial ago. They saw each object 10 times. With feedback              
- The last two runs are explicit categorization task: the third and fourth runs, they saw objects from both categories. Their task is to indicate which catgeory the current object belongs to by pressing 1 or 2. They saw each object 7 times. With feedback       
...
totalData$accuracy[is.na(totalData$accuracy)] =0
...
##Task structure
```{r}
temptrialNumbers=totalData %>%filter(run==3 |run==4) %>%group_by(subID,Cat)%>%summarize(num_same = length(Cat))
head(temptrialNumbers)

temptrialNumbers=totalData %>%filter(run==1 |run==2) %>%group_by(subID,SameDiffImg)%>%summarize(num_same = length(Cat))
head(temptrialNumbers)
```
...
basic = totalData %>% filter(run==3|run==4) %>%group_by(subID,run)%>% summarize(
