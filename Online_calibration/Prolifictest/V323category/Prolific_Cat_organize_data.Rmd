...
for (ifiles in 1:length(Run01_fnames)){
  ...
  for (ifilesDe in 1:length(Demo_fnames)){
      if (unlist(strsplit(Run01_fnames[ifiles],runFnamepattern))[2]==unlist(strsplit(Demo_fnames[ifilesDe],"demographics"))[2]){  tempDemoFile = read.table(paste0(dataPath,Demo_fnames[ifilesDe]),header = F)
  ...
  feature_index = data.frame()
    ...
    for (i in 1:length(Trials)){
      ...
      keyNum = rbind(keyNum,as.numeric(temp[3]))
      accuracy = rbind(accuracy,as.numeric(temp[4]))
      RT = rbind(RT,as.numeric(temp[5]))
      Fir_img = rbind(Fir_img,as.numeric(temp[6]))
      feature_index = rbind(feature_index,as.numeric(temp[7]))
    }
    data = cbind(keyNum,accuracy,RT, Fir_img,feature_index)
    colnames(data) = c("keys","accuracy", "rt", "Fimg","feature_index")
    data = data[-1,]
    ...
    completeFnames[ic] =unlist(strsplit(Run01_fnames[ifiles],"SameDifferent"))[2]
    ic = ic+1;
    #}else{}
}#for
...
for (ifiles in 1:length(Run02_fnames)){
  ...
  for (ifilesDe in 1:length(Post_fnames)){
    if (unlist(strsplit(Run02_fnames[ifiles],runFnamepattern))[2]==unlist(strsplit(Post_fnames[ifilesDe],"postsurvey"))[2]){tempPostFile = read.table(paste0(dataPath,Post_fnames[ifilesDe]),header = F)
        ###separate feedback and prolific ID
        if (dim(tempPostFile)[2]>1){
          tempPostFile$y = apply( tempPostFile[,] , 1 , paste , collapse = "" )
          Post_info = unlist(strsplit(as.character(tempPostFile$y), ";"))
...
for (ifiles in 1:length(Run03_fnames)){
  for (ifilesDe in 1:length(Post_fnames)){
    if (unlist(strsplit(Run03_fnames[ifiles],"CatRun03"))[2]==unlist(strsplit(Post_fnames[ifilesDe],"postsurvey"))[2]){tempPostFile = read.table(paste0(dataPath,Post_fnames[ifilesDe]),header = F)
        ###separate feedback and prolific ID
        if (dim(tempPostFile)[2]>1){
          tempPostFile$y = apply( tempPostFile[,] , 1 , paste , collapse = "" )
          Post_info = unlist(strsplit(as.character(tempPostFile$y), ";"))
...
for (ifiles in 1:length(Run04_fnames)){
  for (ifilesDe in 1:length(Post_fnames)){
    if (unlist(strsplit(Run04_fnames[ifiles],"CatRun04"))[2]==unlist(strsplit(Post_fnames[ifilesDe],"postsurvey"))[2]){tempPostFile = read.table(paste0(dataPath,Post_fnames[ifilesDe]),header = F)
        ###separate feedback and prolific ID
        if (dim(tempPostFile)[2]>1){
          tempPostFile$y = apply( tempPostFile[,] , 1 , paste , collapse = "" )
          Post_info = unlist(strsplit(as.character(tempPostFile$y), ";"))
