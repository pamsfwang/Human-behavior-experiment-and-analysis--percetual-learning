...
for c = 1:length(candidate)
    ...
    %% Testing
    %%category feature for testing
    newparamA = zeros(size(paramA));
    newparamB = zeros(size(paramB));
    
    for ii = 1:length(New_level)
      newparamA(find(paramA==F1_level(ii)))=New_level(ii);  
      newparamB(find(paramB==F1_level(ii)))=New_level(ii);  
    end
    
    %%object feature for testing
    Newtemp3A = ff3(1).orig;
    Newtemp3B = ff3(2).orig;
    
    NewparamA = [newparamA Newtemp3A(randperm(length(Newtemp3A)))'];
    NewparamB = [newparamB Newtemp3B(randperm(length(Newtemp3B)))'];
