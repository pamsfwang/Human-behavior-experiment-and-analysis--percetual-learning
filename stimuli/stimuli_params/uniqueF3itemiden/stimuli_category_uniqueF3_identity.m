%Shao-Fang Wang 2018
%file name
fname = 'stimuli_param_uniqueF3_itemiden.csv';
fname_train = 'stimuli_param_uniqueF3_train_itemiden.csv';
fname_test = 'stimuli_param_uniqueF3_test_itemiden.csv';
...
[H,P,CI,STATS]=ttest2(CatA_train_feature_set(:,3),CatB_train_feature_set(:,3));
fprintf('Check feature03 randomness of training set: t-test: pvalue %f\n',P)
%%%Plot
...
%%add item identity
temp = eye(size(Cat_train_set,1));
Cat_train_set = [Cat_train_set temp];
...
[H,P,CI,STATS]=ttest2(CatA_new(:,3),CatB_new(:,3));
fprintf('Check feature03 randomness of new set: t-test: pvalue %f\n',P)
%%add item identity
temp = eye(size(New_set,1));
...
% New_set = [New_set zeros(size(New_set,1),1)];
 Cat = [Cat_train_set(:,1:3); New_set(:,1:3)];
% 
%%%Plot
for ii = 10:10:100
